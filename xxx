		...
		if CanRead == nil then
			...
			if ok then
				...
				if specialProperties == nil and getspecialinfo then
					-- Check if instance is of valid type before passing it to getspecialinfo
if instance:IsA("MeshPart") or instance:IsA("UnionOperation") or instance:IsA("Terrain") then
    specialProperties = getspecialinfo(instance)
else
    print("Invalid argument: expected MeshPart, UnionOperation, or Terrain")
end

					-- Ensure specialProperties is initialized before accessing its properties
if specialProperties then
    raw = specialProperties[PropertyName]
    if not raw then
        print("Property with name " .. PropertyName .. " not found.")
    end
else
    print("specialProperties is nil.")
end
	...
	local function savegame()
		...
		if OPTIONS.ReadMe then
			...
			If Union and MeshPart collisions don't work, run the script below in the Studio Command Bar:
					if instance:IsA("MeshPart") or instance:IsA("UnionOperation") or instance:IsA("Terrain") then
    -- the code which does stuff with them
